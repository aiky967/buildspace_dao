...
const App = () => {
  ...
  // Withou ot we can only read data, not write.
  const signer = provider ? provider.getSigner() : undefined;

  // State variable for us to know if user has our NFT.
  const [hasClaimedNFT, setHasClaimedNFT] = useState(false);

  // isClaiming lets us easily keep a loading state while the NFT is miniting.
  ...
  useEffect(async () => {
    ...
    // A simple call to voteModule.geAll() to grab the proposals.
    try {
  ...
  useEffect(async () => {
    ...
    try {
      ...
      console.error("failed toi get member list", error);
    }
  }, [hasClaimedNFT]);
  ...
  // Now we combine the memberAddresses and memberTokenAmounts also into a single array
  ...
  // When user hasn't connected their wallet
  // to your web app. Let them calll connectWallet.
  if(!address) {
    return (
